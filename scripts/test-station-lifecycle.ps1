...
$subUrl = if ([string]::IsNullOrWhiteSpace($env:RELAYHUB_E2E_SUB2_URL)) { "https://chat.178266.xyz" } else { $env:RELAYHUB_E2E_SUB2_URL }
$newUrl = if ([string]::IsNullOrWhiteSpace($env:RELAYHUB_E2E_NEWAPI_URL)) { "https://sadai.cc" } else { $env:RELAYHUB_E2E_NEWAPI_URL }
...
try {
  ...
  $subName = "relayhub-e2e-$([guid]::NewGuid().ToString('N').Substring(0, 16))"
  ...
  $subUpdatedName = "$subName-updated"
  try { $null = Json-Request PATCH "$subUrl/api/v1/keys/$subKeyId" $subHeaders @{ name = $subUpdatedName } $null }
  catch { $null = Json-Request PUT "$subUrl/api/v1/keys/$subKeyId" $subHeaders @{ name = $subUpdatedName } $null }
  $subKey = (Json-Request GET "$subUrl/api/v1/keys/$subKeyId" $subHeaders $null $null).data
  if ([string]$subKey.name -ne $subUpdatedName) { throw "Sub2API key name update was not persisted" }
  foreach ($status in @("inactive", "active")) {
    try { $null = Json-Request PATCH "$subUrl/api/v1/keys/$subKeyId" $subHeaders @{ status = $status } $null }
    catch { $null = Json-Request PUT "$subUrl/api/v1/keys/$subKeyId" $subHeaders @{ status = $status } $null }
    $subKey = (Json-Request GET "$subUrl/api/v1/keys/$subKeyId" $subHeaders $null $null).data
    if ([string]$subKey.status -ne $status) { throw "Sub2API key status '$status' was not persisted" }
  }
  ...
  $newName = "relayhub-e2e-$([guid]::NewGuid().ToString('N').Substring(0, 16))"
  ...
  $newUpdatedName = "$newName-updated"
  $token.name = $newUpdatedName
  $null = Json-Request PUT "$newUrl/api/token/" $newHeaders $token $newSession
  $token = (Json-Request GET "$newUrl/api/token/$newKeyId" $newHeaders $null $newSession).data
  if ([string]$token.name -ne $newUpdatedName) { throw "NewAPI key name update was not persisted" }
  $null = Json-Request PUT "$newUrl/api/token/?status_only=true" $newHeaders @{ id = $token.id; status = 2 } $newSession
  $token = (Json-Request GET "$newUrl/api/token/$newKeyId" $newHeaders $null $newSession).data
  if ([int]$token.status -ne 2) { throw "NewAPI key disable was not persisted" }
  $null = Json-Request PUT "$newUrl/api/token/?status_only=true" $newHeaders @{ id = $token.id; status = 1 } $newSession
  $token = (Json-Request GET "$newUrl/api/token/$newKeyId" $newHeaders $null $newSession).data
  if ([int]$token.status -ne 1) { throw "NewAPI key enable was not persisted" }
}
finally {
