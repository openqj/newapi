...
impl LoginProfileStore for Store {
    fn list_login_profiles(&self) -> Result<Vec<LoginProfile>, String> {
        let mut statement = self.connection.prepare("SELECT id,name,username FROM login_profiles ORDER BY name").map_err(|error| error.to_string())?;
    ...
    fn save_login_profile(&self, profile: &LoginProfile) -> Result<(), String> {
        self.connection.execute("INSERT OR REPLACE INTO login_profiles (id,name,username) VALUES (?1,?2,?3)", params![profile.id, profile.name, profile.username]).map_err(|error| error.to_string())?;
        Ok(())
    }

    fn delete_login_profile(&self, id: &str) -> Result<(), String> {
        self.connection.execute("DELETE FROM login_profiles WHERE id=?1", [id]).map_err(|error| error.to_string())?;
        Ok(())
    }
}
