...
fn remote_server(row: &Row<'_>) -> rusqlite::Result<RemoteServer> {
    Ok(RemoteServer {
        ...
        codex_latest_version: row.get(8)?,
        codex_update_available: row.get(9)?,
        host_key_fingerprint: row.get(10)?,
        relay_url: row.get(11)?,
        relay_provider: row.get(12)?,
        relay_key_source: row.get(13)?,
        relay_key_masked: row.get(14)?,
        relay_config_fingerprint: row.get(15)?,
        connection_status: row.get(16)?,
        connection_error: row.get(17)?,
        last_synced_at: row.get(18)?,
        last_sync_status: row.get(19)?,
        last_sync_error: row.get(20)?,
...
impl RemoteServerStore for Store {
    fn list_remote_servers(&self) -> Result<Vec<RemoteServer>, String> {
        let mut statement = self
            .connection
            .prepare(&format!(
                "SELECT {REMOTE_SERVER_COLUMNS} FROM remote_servers ORDER BY name"
            ))
            .map_err(|error| error.to_string())?;
        let servers = statement
            .query_map([], remote_server)
            .map_err(|error| error.to_string())?
    ...
    fn get_remote_server(&self, id: &str) -> Result<RemoteServer, String> {
        self.connection
            .query_row(
                &format!("SELECT {REMOTE_SERVER_COLUMNS} FROM remote_servers WHERE id=?1"),
                [id],
                remote_server,
            )
            .map_err(|error| error.to_string())
    ...
    fn delete_remote_server(&self, id: &str) -> Result<(), String> {
        self.connection
            .execute("DELETE FROM remote_servers WHERE id=?1", [id])
            .map_err(|error| error.to_string())?;
        self.connection
            .execute("DELETE FROM remote_sync_logs WHERE server_id=?1", [id])
            .map_err(|error| error.to_string())?;
        Ok(())
    }
}
