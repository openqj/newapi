...
impl StationSnapshotStore for Store {
    fn load_snapshot(&self, id: &str) -> Result<Option<(String, StationSnapshot)>, String> {
        let result = self.connection.query_row(
            "SELECT fingerprint,payload FROM snapshots WHERE station_id=?1",
            [id],
            |row| Ok((row.get::<_, String>(0)?, row.get::<_, String>(1)?)),
        );
        match result {
            Ok((fingerprint, payload)) => Ok(Some((
                fingerprint,
    ...
    fn save_snapshot(
        ...
        for change in changes {
            self.connection
                .execute(
                    "INSERT INTO changes (station_id,summary,created_at) VALUES (?1,?2,?3)",
                    params![id, change, timestamp],
                )
    ...
    fn history(&self, id: &str) -> Result<Vec<Value>, String> {
        ...
        let history = statement
            .query_map([id], |row| {
                Ok(json!({"summary": row.get::<_, String>(0)?, "createdAt": row.get::<_, i64>(1)?}))
            })
            .map_err(|error| error.to_string())?
