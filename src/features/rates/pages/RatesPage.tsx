...
export function RatesPage({ rows, stations, unavailableStationCount, onRefresh, onOpenStation }: { rows: RateRow[]; stations: Station[]; unavailableStationCount: number; onRefresh: () => Promise<void>; onOpenStation: (url: string) => void | Promise<void> }) {
  ...
  const sorted = [...filtered].sort((left, right) => (left.rate.multiplier - right.rate.multiplier) * (sortDirection === "asc" ? 1 : -1) || rateModelLabel(left).localeCompare(rateModelLabel(right), "zh-CN") || left.stationName.localeCompare(right.stationName, "zh-CN"));
