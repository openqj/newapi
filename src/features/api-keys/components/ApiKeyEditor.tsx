...
import { Button, FormDialog, SelectField, Switch, TextareaField, TextField } from "../../../components/ui";
...
export function ApiKeyEditor({ row, rows, stations, onRefreshStation, onClose, onSaved, onError }: ApiKeyEditorProps) {
  ...
  return (
    <FormDialog
      ...
      footer={<><Button variant="secondary" className="form-dialog-cancel" onClick={onClose} disabled={saving}>取消</Button><Button variant="primary" className="form-dialog-submit" type="submit" disabled={saving || !stationId || !name.trim()}>{saving && <RefreshCw size={16} className="sub2-spin" />}{row ? "保存" : "创建"}</Button></>}
    >
      {!row && <label>来源站点<SelectField value={stationId} disabled={saving} onChange={(event) => { setStationId(event.target.value); setGroup(""); }}>{stations.map((station) => <option value={station.id} key={station.id}>{station.name} / {station.baseUrl}</option>)}</SelectField></label>}
      <label>密钥名称<TextField autoFocus value={name} onChange={(event) => setName(event.target.value)} placeholder="请输入密钥名称" /></label>
      {!row && <div className="sub2-key-name-presets" aria-label="密钥名称预设">{keyNamePresets.map((preset) => <Button variant={name === preset ? "primary" : "secondary"} onClick={() => setName(preset)} key={preset}>{preset}</Button>)}</div>}
      ...
      {!row && !isNewApi && <section className="sub2-editor-section"><ToggleRow label="自定义密钥" checked={useCustomKey} onChange={setUseCustomKey} />{useCustomKey && <><TextField value={customKey} onChange={(event) => setCustomKey(event.target.value)} className="sub2-mono-input" placeholder="请输入自定义密钥" /><small>至少 8 个字符，仅允许字母、数字、连字符和下划线。</small></>}</section>}
      {row && <label>状态<SelectField value={status} onChange={(event) => setStatus(event.target.value)}><option value="active">启用</option><option value="inactive">停用</option></SelectField></label>}
      <section className="sub2-editor-section"><ToggleRow label="IP 限制" checked={enableIpRestriction} onChange={setEnableIpRestriction} />{enableIpRestriction && <div className="sub2-editor-stack"><label>IP 白名单<TextareaField value={whitelist} onChange={(event) => setWhitelist(event.target.value)} placeholder="每行一个 IP 地址" /></label>{!isNewApi && <label>IP 黑名单<TextareaField value={blacklist} onChange={(event) => setBlacklist(event.target.value)} placeholder="每行一个 IP 地址" /></label>}{isNewApi && <small>该 NewAPI 站点仅支持 IP 白名单。</small>}</div>}</section>
      <section className="sub2-editor-section"><label>额度上限<TextField value={quota} inputMode="decimal" onChange={(event) => setQuota(event.target.value)} placeholder="0" /><small>填写 0 或留空表示不限额。</small></label>{row?.key.usedQuota != null && <div className="sub2-editor-readonly"><span>已用额度</span><strong>{formatMoney(row.key.usedQuota)} / {formatMoney((row.key.usedQuota ?? 0) + (row.key.remainingQuota ?? 0))}</strong></div>}</section>
      ...
      <section className="sub2-editor-section"><ToggleRow label="过期时间" checked={enableExpiration} onChange={setEnableExpiration} />{enableExpiration && <div className="sub2-editor-stack"><div className="sub2-expiration-presets">{["7", "30", "90"].map((days) => <Button variant="ghost" className={expiresInDays === days ? "active" : ""} onClick={() => setExpirationPreset(days)} key={days}>{days} 天</Button>)}<Button variant="ghost" className={!['7', '30', '90'].includes(expiresInDays) ? "active" : ""} onClick={() => setExpiresInDays("")}>自定义</Button></div><label>有效期（天）<TextField value={expiresInDays} inputMode="numeric" onChange={(event) => setExpiresInDays(event.target.value)} placeholder="请输入天数" /></label></div>}</section>
    </FormDialog>
  );
}

function ToggleRow({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) {
  return <div className="sub2-toggle-row"><span>{label}</span><Switch label={label} checked={checked} onCheckedChange={onChange} /></div>;
}

function RateLimitField({ label, value, onChange }: { label: string; value: string; onChange: (value: string) => void }) {
  return <label>{label}<TextField value={value} inputMode="decimal" onChange={(event) => onChange(event.target.value)} placeholder="0" /></label>;
}
