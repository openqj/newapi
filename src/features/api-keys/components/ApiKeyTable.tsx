    ...
    <DataTable
      ...
      desktop={<div className={`sub2-desktop-table table-page-data-table sub2-key-data-table ${hiddenColumns}`}>
        <table>
          <thead>
            <tr>
              <th>中转站</th><th>名称</th><th>API 密钥</th><th>分组</th><th>当前并发</th>
              <th>用量</th><th>过期时间</th><th>状态</th><th>创建时间</th><th>操作</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((row) => {
              ...
              return (
                <tr key={`${row.stationId}:${row.key.id}`}>
                  ...
                  <td><StatusBadge status={active ? "online" : "partial"} /></td>
      ...
      mobile={<div className="sub2-mobile-cards">
        {rows.map((row) => {
          const active = isActive(row.key.status);
          return <article className="sub2-record-card" key={`${row.stationId}:${row.key.id}`}>
            <div><strong>{row.key.name || "未命名密钥"}</strong><StatusBadge status={active ? "online" : "partial"} /></div>
            <code>{row.key.maskedKey || "已隐藏"}</code>
            <dl>
              <div><dt>来源</dt><dd>{row.stationName}</dd></div><div><dt>网址</dt><dd>{row.stationUrl}</dd></div>
