        ...
        <table>
          ...
          <tbody>
            {rows.map((row) => {
              ...
              const busy = saving === id || saving === `codex:${id}` || testState?.status === "testing";
              return (
                <tr key={id}>
                  ...
                  <td><div className="sub2-key-status" title={testState?.message}><StatusBadge status={active ? "online" : "partial"} /><TestStatus state={testState} /></div></td>
