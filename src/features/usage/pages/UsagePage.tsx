...
import { Activity, ChevronLeft, ChevronRight, LayoutDashboard, RefreshCw, Settings2, Timer, TriangleAlert } from "lucide-react";
import {
  ArcElement,
  CategoryScale,
  Chart as ChartJS,
...
function EmptyState({ message }: { message: string }) {
  return <div className="sub2-empty"><TriangleAlert size={22} /><span>{message}</span></div>;
}
...
export function UsagePage({ rows, stations, onRefresh }: { rows: UsageLog[]; stations: Station[]; onRefresh: () => Promise<void> }) {
  ...
  return <div className="sub2-page sub2-usage-page">
    ...
    <section className="sub2-panel sub2-table-panel data-table"><div className="sub2-result-line">共 <strong>{filtered.length}</strong> 条记录</div><UsageRecordsTable rows={pageRows} columns={columns} /><Pagination page={activePage} pages={pages} onChange={setPage} /></section>
  </div>;
}
