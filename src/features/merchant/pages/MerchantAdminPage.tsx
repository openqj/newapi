...
import { Copy, Eye, EyeOff, Filter, Pencil, Pin, Plus, RefreshCw, Search, ShieldCheck, Trash2 } from "lucide-react";
import { Button, FormDialog, FormField, IconButton, Pagination, Panel, SelectField, TextareaField, TextField, useConfirm, useToast } from "../../../components/ui";
...
export function MerchantAdminPage() {
  ...
  return <div className="merchant-admin-page">
    <Panel title="副窗口商家信息管理" description="维护商家信息副窗口中的分组倍率和免费额度；置顶内容会固定显示在列表最前。">
      <div className="merchant-admin-toolbar"><span className="merchant-admin-toolbar-status">{demoMode && <span className="merchant-demo-badge">模拟数据，仅本机预览</span>}{profiles.length ? `已识别 ${profiles.length} 个商家资料` : "请先由商家保存资料后再新增列表项"}</span><Button variant="secondary" onClick={() => void load()} disabled={loading || Boolean(savingId)}><RefreshCw size={16} className={loading ? "sub2-spin" : ""} />刷新</Button></div>
    </Panel>
    {demoMode && <div className="merchant-demo-notice" role="status"><span>当前展示的是本机模拟数据，仅供预览，不会写入线上商家数据。</span></div>}
    {loadError && <div className="merchant-load-error merchant-admin-load-error" role="alert"><span>线上商家数据加载失败：{loadError}</span><Button variant="secondary" onClick={() => void load()} disabled={loading || Boolean(savingId)}><RefreshCw size={14} />重试</Button></div>}

    <Panel className="merchant-admin-management-card">
      <nav className="merchant-admin-tabs" role="tablist" aria-label="商家信息管理分类">
        <Button variant="ghost" className={`merchant-admin-tab ${managementTab === "profiles" ? "active" : ""}`} role="tab" aria-selected={managementTab === "profiles"} aria-controls="merchant-admin-profiles" onClick={() => setManagementTab("profiles")}>商家资料</Button>
        <Button variant="ghost" className={`merchant-admin-tab ${managementTab === "rates" ? "active" : ""}`} role="tab" aria-selected={managementTab === "rates"} aria-controls="merchant-admin-rates" onClick={() => setManagementTab("rates")}>分组倍率列表</Button>
        <Button variant="ghost" className={`merchant-admin-tab ${managementTab === "accounts" ? "active" : ""}`} role="tab" aria-selected={managementTab === "accounts"} aria-controls="merchant-admin-accounts" onClick={() => setManagementTab("accounts")}>免费额度列表</Button>
      </nav>
      ...
      {managementTab === "rates" && <section id="merchant-admin-rates" className="merchant-admin-tab-panel" role="tabpanel">
        ...
        <div className="merchant-admin-table-wrap"><table><thead><tr><th>商家</th><th>站点 / 分组</th><th>倍率</th><th>排序</th><th>操作</th></tr></thead><tbody>{filteredRates.length ? visibleRates.map((item) => <tr key={item.id}><td data-label="商家"><strong>{item.merchantName}</strong></td><td data-label="站点 / 分组"><strong>{item.stationName}</strong><small>{item.groupName} · {item.stationUrl}</small></td><td data-label="倍率">{item.multiplierSummary}</td><td data-label="排序"><Button variant="secondary" className="merchant-admin-pin-button" onClick={() => void saveRate(rateInput(item, !item.pinned))} disabled={Boolean(savingId)}><Pin size={14} />{item.pinned ? "取消置顶" : "置顶"}</Button></td><td data-label="操作"><div className="merchant-admin-row-actions"><IconButton variant="secondary" label={`编辑 ${item.merchantName} 的倍率`} onClick={() => setEditingRate(item)} disabled={Boolean(savingId)} icon={<Pencil size={14} />} /><IconButton variant="secondary" label={`删除 ${item.merchantName} 的倍率`} onClick={() => void deleteRate(item)} disabled={Boolean(savingId)} icon={<Trash2 size={14} />} /></div></td></tr>) : <tr><td colSpan={5}>{loading ? "正在加载…" : "没有符合条件的分组倍率。"}</td></tr>}</tbody></table></div>
        <Pagination page={page} pageCount={totalPages} onPageChange={setPage} ariaLabel="商户管理分页" className="merchant-admin-pagination" />
      </section>}
      {managementTab === "accounts" && <section id="merchant-admin-accounts" className="merchant-admin-tab-panel" role="tabpanel">
        <div className="merchant-admin-tab-heading"><div><h3>免费额度列表</h3><p>管理商家提供的兑换码；用户导入时使用自己的站点登录账号完成兑换。</p></div><Button variant="primary" onClick={() => setEditingAccount("new")} disabled={!profiles.length || Boolean(savingId)}><Plus size={16} />新增兑换码</Button></div>
        ...
        <div className="merchant-admin-table-wrap"><table><thead><tr><th>商家</th><th>站点</th><th>兑换码</th><th>免费额度</th><th>状态</th><th>排序</th><th>操作</th></tr></thead><tbody>{filteredAccounts.length ? visibleAccounts.map((item) => { const revealed = revealedCodes[item.id]; return <tr key={item.id}><td data-label="商家"><strong>{item.merchantName}</strong></td><td data-label="站点"><strong>{item.stationName}</strong><small>{item.stationUrl}</small></td><td data-label="兑换码"><div className="merchant-admin-code-cell"><code>{revealed ?? item.redeemCodeMasked ?? maskRedeemCode(item.redeemCode)}</code><div className="merchant-admin-code-actions"><IconButton variant="secondary" label={`${revealed ? "隐藏" : "查看"} ${item.merchantName} 的兑换码`} onClick={() => { if (revealed) setRevealedCodes((current) => { const next = { ...current }; delete next[item.id]; return next; }); else void revealCode(item, "view"); }} disabled={Boolean(savingId)} icon={revealed ? <EyeOff size={14} /> : <Eye size={14} />} /><IconButton variant="secondary" label={`复制 ${item.merchantName} 的兑换码`} onClick={() => void copyCode(item)} disabled={Boolean(savingId)} icon={<Copy size={14} />} /></div></div></td><td data-label="免费额度">{item.quota.toFixed(2)} 元</td><td data-label="状态">{item.claimed ? "已领取" : "可导入"}</td><td data-label="排序"><Button variant="secondary" className="merchant-admin-pin-button" onClick={() => void saveAccount(freeCodeInput(item, !item.pinned))} disabled={Boolean(savingId)}><Pin size={14} />{item.pinned ? "取消置顶" : "置顶"}</Button></td><td data-label="操作"><div className="merchant-admin-row-actions"><IconButton variant="secondary" label={`编辑 ${item.merchantName} 的免费额度`} onClick={() => void openAccountEditor(item)} disabled={Boolean(savingId)} icon={<Pencil size={14} />} /><IconButton variant="secondary" label={`删除 ${item.merchantName} 的免费额度`} onClick={() => void deleteAccount(item)} disabled={Boolean(savingId)} icon={<Trash2 size={14} />} /></div></td></tr>; }) : <tr><td colSpan={7}>{loading ? "正在加载…" : "没有符合条件的免费额度。"}</td></tr>}</tbody></table></div>
        <Pagination page={page} pageCount={totalPages} onPageChange={setPage} ariaLabel="商户管理分页" className="merchant-admin-pagination" />
      </section>}
    </Panel>
...
function MerchantListFilters({
  ...
  return <div className="merchant-admin-filters">
    <div className="merchant-admin-filter-fields">
      ...
      <label className="merchant-admin-filter-field"><span>商家</span><SelectField aria-label="商家筛选" value={merchantFilter} onChange={(event) => onMerchantFilter(event.target.value)}><option value="all">全部商家</option>{profiles.map((profile) => <option key={profile.userId} value={profile.userId}>{profile.merchantName}</option>)}</SelectField></label>
      <label className="merchant-admin-filter-field"><span>状态</span><SelectField aria-label="状态筛选" value={statusFilter} onChange={(event) => onStatusFilter(event.target.value as MerchantStatusFilter)}><option value="all">全部状态</option>{statusOptions.map((option) => <option key={option.value} value={option.value}>{option.label}</option>)}</SelectField></label>
    </div>
    <div className="merchant-admin-filter-meta"><span><Filter size={14} aria-hidden="true" />显示 {filtered} / {total} 条</span>{hasFilters && <Button variant="secondary" onClick={onReset}><Filter size={14} />清除筛选</Button>}</div>
  </div>;
}
...
function ProfileEditor({ profile, saving, onClose, onSave }: { profile: AdminMerchantProfile; saving: boolean; onClose: () => void; onSave: (input: AdminMerchantProfileInput) => Promise<void> }) {
  ...
  return <FormDialog title="编辑商家资料" description="这些资料会显示在副窗口的联系弹窗中。" ariaLabel="商家资料" onClose={onClose} onSubmit={submit} footer={<><Button variant="secondary" onClick={onClose} disabled={saving}>取消</Button><Button variant="primary" type="submit" disabled={saving || !merchantName.trim()}>{saving ? "保存中" : "保存"}</Button></>}>
