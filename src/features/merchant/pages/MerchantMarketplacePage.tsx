...
function MerchantInfoDialog({ merchant, onClose }: { merchant: MerchantProfile; onClose: () => void }) {
  return <FormDialog title={merchant.merchantName} description={merchant.description || "暂无商家说明"} ariaLabel={`${merchant.merchantName} 商家信息`} onClose={onClose} footer={<button type="button" className="button-secondary" onClick={onClose}>关闭</button>}>
    <div className="merchant-contact-dialog">
      <div className="merchant-qq-contact"><span>QQ</span><strong>{merchant.qq || "未填写"}</strong>{merchant.qqLink && <button type="button" className="button-primary" onClick={() => void external(merchant.qqLink!)}>打开联系链接</button>}</div>
