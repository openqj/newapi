...
export function AlertSettings() {
  ...
  return <div className="flex flex-col gap-3 border-b border-slate-100 p-4 last:border-b-0">
    ...
    <div className="flex gap-2"><button type="button" className="button-primary" onClick={() => void save()} disabled={saving || !isTauri()}>{saving ? "保存中" : "保存告警策略"}</button><button type="button" className="button-secondary" onClick={() => void evaluate()} disabled={!isTauri()}>立即评估</button></div>
  </div>;
}
