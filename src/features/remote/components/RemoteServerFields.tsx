  ...
  return (
    <>
      <label className="remote-name-field">
        显示名称
        <TextField className="mt-1" name="name" defaultValue={server?.name} placeholder="可选" />
      </label>
      ...
      <label className="remote-port-field">
        ...
        <TextField className="mt-1" name="port" type="number" min="1" max="65535" required defaultValue={server?.port ?? 22} />
      </label>
      ...
      <div className="remote-auth-tabs" role="tablist" aria-label="认证方式">
        <Button variant="test" className={authType === "password" ? "active" : ""} role="tab" aria-selected={authType === "password"} onClick={() => onAuthTypeChange("password")}>密码</Button>
        <Button variant="test" className={authType === "key" ? "active" : ""} role="tab" aria-selected={authType === "key"} onClick={() => onAuthTypeChange("key")}>身份文件</Button>
      </div>
      {authType === "password" ? (
        ...
        <label className="remote-credential-field">
          ...
          <div className="secret-input-wrap mt-1">
            <TextField
              ...
              onInput={(event) => event.currentTarget.setCustomValidity("")}
            />
            <IconButton variant="ghost" className="secret-file-button" label="选择电脑文件" onClick={onChoosePrivateKey} icon={<FolderOpen size={17} />} />
          </div>
        </label>
      )}
    </>
