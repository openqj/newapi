...
export function RemoteServerRow({
  ...
  return <tr>
    ...
    <td>{editingUrl ? <div className="relay-key-input" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) onCancelRelayEdit("url"); }}><input className="input relay-input" autoFocus value={relayDraft.url} onChange={(event) => onRelayDraftChange({ url: event.target.value })} placeholder="输入中转站地址" /><button type="button" className="icon-button" aria-label="保存中转站地址" title="保存中转配置" disabled={savingRelay} onClick={onSaveRelay}><Check size={16} /></button></div> : <button type="button" className="relay-display" onClick={() => onStartRelayEdit("url")}>{server.relayUrl ?? "未配置"}</button>}</td>
    <td>{editingKey ? <div className="relay-key-input" onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget as Node)) onCancelRelayEdit("key"); }}><input className="input" autoFocus type="password" value={relayDraft.key} onChange={(event) => onRelayDraftChange({ key: event.target.value })} placeholder={server.relayKeyMasked ? "已安全保存，输入新密钥以替换" : "输入中转站密钥"} /><button type="button" className="icon-button" aria-label="保存 API 密钥" title="保存中转配置" disabled={savingRelay} onClick={onSaveRelay}><Check size={16} /></button></div> : <button type="button" className="relay-display relay-key-display api-key-mask" onClick={() => onStartRelayEdit("key")}>{server.relayKeyMasked ?? "未配置"}</button>}</td>
