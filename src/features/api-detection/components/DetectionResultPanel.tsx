  ...
  return <section className="api-detection-result" aria-live="polite">
    ...
    <div className="api-detection-result-body">
      ...
      <div className="api-detection-checks">{checks.map((check, index) => {
        ...
        return <div className={`api-detection-check ${check.status}`} key={`${check.name}-${index}`}><Icon className="api-detection-check-status-icon" size={17} /><LeadingIcon className="api-detection-check-type-icon" size={15} /><div><strong>{checkLabels[checkName] ?? check.name}</strong><p>{check.detail}</p>{check.trace && <><Button variant="ghost" className="api-detection-trace-button" onClick={() => onToggleTrace(check.name)}>响应摘要 <ChevronDown size={14} /></Button>{expandedTrace === check.name && <pre>{check.trace}</pre>}</>}</div><b>{statusLabel(check.status)}</b></div>;
      })}</div>
    </div>
    {(result.source || result.behavior) && <section className="api-detection-evidence">
      {result.source && <div className="api-detection-evidence-column"><header><div><span>来源层</span><strong>{sourceLabel(result.source.classification)} · {result.source.score}%</strong></div><b>{Math.round(result.source.confidence * 100)}% 证据置信度</b></header><List className="api-detection-evidence-list">{result.source.signals.map((signal) => <ListItem key={signal.id} className={signal.status}><strong>{signal.name}</strong><span>{signal.detail}</span><b>{statusLabel(signal.status)}</b></ListItem>)}</List></div>}
      {result.behavior && <div className="api-detection-evidence-column"><header><div><span>行为指纹层</span><strong>{result.behavior.score}%</strong></div><b>重复置信度 {Math.round(result.behavior.confidence * 100)}%</b></header><List className="api-detection-evidence-list">{result.behavior.probes.map((probe) => <ListItem key={probe.id} className={probe.status}><strong>{probe.name}</strong><span>{probe.detail}</span><b>{statusLabel(probe.status)}</b></ListItem>)}</List></div>}
    </section>}
