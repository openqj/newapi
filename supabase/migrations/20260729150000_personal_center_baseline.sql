-- RelayHub personal-center schema baseline. Apply first on an empty project.
...
create index if not exists personal_center_memberships_user_id_idx on public.personal_center_memberships (user_id);
create index if not exists personal_center_audit_events_created_at_idx on public.personal_center_audit_events (created_at desc);
create index if not exists personal_center_notifications_target_idx on public.personal_center_notifications (target_user_id, published_at desc);
create index if not exists notification_receipts_user_idx on public.notification_receipts (user_id, read_at);
create index if not exists personal_center_login_events_created_at_idx on public.personal_center_login_events (created_at desc);
...
begin
  ...
  where lower(email) = lower(trim(new.user_email)) limit 1;
...
begin
  if tg_table_name = 'personal_center_memberships' then
    if tg_op = 'DELETE' then
      insert into public.personal_center_audit_events (actor_id, target_user_id, action, subject, detail)
      values (auth.uid(), old.user_id, 'membership_delete', old.station_id || ':' || old.account_id, old.plan || ' / ' || old.access_level);
    else
      insert into public.personal_center_audit_events (actor_id, target_user_id, action, subject, detail)
      values (auth.uid(), new.user_id, 'membership_' || lower(tg_op), new.station_id || ':' || new.account_id, new.plan || ' / ' || new.access_level);
    end if;
  else
    insert into public.personal_center_audit_events (actor_id, action, subject, detail)
    values (auth.uid(), 'notification_preferences_' || lower(tg_op), 'notifications', 'Updated global notification preferences');
  end if;
...
begin
  ...
  if new.audience = 'all' then
    ...
    select id, lower(email) into new.target_user_id, new.target_email from auth.users
    where lower(email) = lower(trim(new.target_email)) limit 1;
...
create trigger relayhub_resolve_membership_user before insert or update on public.personal_center_memberships
...
create trigger relayhub_touch_notification_preferences before update on public.personal_center_notification_preferences
...
create trigger relayhub_audit_membership after insert or update or delete on public.personal_center_memberships
...
create trigger relayhub_audit_notification_preferences after update on public.personal_center_notification_preferences
for each row execute function public.relayhub_record_personal_center_audit();
drop trigger if exists relayhub_prepare_notification on public.personal_center_notifications;
create trigger relayhub_prepare_notification before insert on public.personal_center_notifications
...
create trigger relayhub_prepare_notification_receipt before insert or update on public.notification_receipts
...
create policy "Authenticated users read global notification preferences" on public.personal_center_notification_preferences for select to authenticated using (true);
...
create policy "Administrators manage global notification preferences" on public.personal_center_notification_preferences for all to authenticated using (public.relayhub_is_admin()) with check (public.relayhub_is_admin());
drop policy if exists "Users read their memberships" on public.personal_center_memberships;
create policy "Users read their memberships" on public.personal_center_memberships for select to authenticated using (user_id = auth.uid() or public.relayhub_is_admin());
drop policy if exists "Administrators manage memberships" on public.personal_center_memberships;
create policy "Administrators manage memberships" on public.personal_center_memberships for all to authenticated using (public.relayhub_is_admin()) with check (public.relayhub_is_admin());
drop policy if exists "Users read relevant audit events" on public.personal_center_audit_events;
create policy "Users read relevant audit events" on public.personal_center_audit_events for select to authenticated using (target_user_id = auth.uid() or public.relayhub_is_admin());
drop policy if exists "Users read addressed notifications" on public.personal_center_notifications;
create policy "Users read addressed notifications" on public.personal_center_notifications for select to authenticated
using (((audience = 'all' or target_user_id = auth.uid()) and (expires_at is null or expires_at > extract(epoch from now())::bigint)) or public.relayhub_is_admin());
drop policy if exists "Administrators publish notifications" on public.personal_center_notifications;
create policy "Administrators publish notifications" on public.personal_center_notifications for insert to authenticated with check (public.relayhub_is_admin());
drop policy if exists "Administrators delete notifications" on public.personal_center_notifications;
create policy "Administrators delete notifications" on public.personal_center_notifications for delete to authenticated using (public.relayhub_is_admin());
drop policy if exists "Users read their notification receipts" on public.notification_receipts;
create policy "Users read their notification receipts" on public.notification_receipts for select to authenticated using (user_id = auth.uid() or public.relayhub_is_admin());
drop policy if exists "Users create their notification receipts" on public.notification_receipts;
create policy "Users create their notification receipts" on public.notification_receipts for insert to authenticated with check (user_id = auth.uid() and exists (select 1 from public.personal_center_notifications notification where notification.id = notification_id and (notification.audience = 'all' or notification.target_user_id = auth.uid()) and (notification.expires_at is null or notification.expires_at > extract(epoch from now())::bigint)));
drop policy if exists "Users update their notification receipts" on public.notification_receipts;
create policy "Users update their notification receipts" on public.notification_receipts for update to authenticated using (user_id = auth.uid()) with check (user_id = auth.uid() and exists (select 1 from public.personal_center_notifications notification where notification.id = notification_id and (notification.audience = 'all' or notification.target_user_id = auth.uid()) and (notification.expires_at is null or notification.expires_at > extract(epoch from now())::bigint)));
drop policy if exists "Administrators read login events" on public.personal_center_login_events;
create policy "Administrators read login events" on public.personal_center_login_events for select to authenticated using (public.relayhub_is_admin());
